#---------------------------------------------------------------------------
# Packer template for building LXD container image for the website-provisioner 
...
# Specify the Hugo version to deploy
variable "hugo_version" {
  description = "Mandatory: The hugo version to use in the image build."
  type        = string
}

# Specify the Webhook version to deploy
...
locals {
  service_name = "hugo-website-provisioner"
}
...
locals {
  ...
  build_playbook_file  = "${abspath(path.root)}/playbooks/provision-website-provisioner.yml"
  build_extra_vars     = "host_id=${var.host_id} hugo_version=${var.hugo_version} webhook_version=${var.webhook_version}"
}
